    ...
    <div class="container flex flex-wrap justify-between items-center mx-auto">
      ...
      <div class="flex items-center md:order-2 space-x-3">
        ...
        <div v-else class="flex flex-row space-x-3">
          <button @click="() => this.$router.push('/register')" class="font-semibold dark:text-white p-2 hover:text-greenlogo duration-200 ease-in">Join</button>
          <button @click="() => this.$router.push('/login')" class="font-semibold dark:text-white p-2 hover:text-greenlogo duration-200 ease-in">Login</button>
        </div>
        <div @click="toggleDarkMode()" class="cursor-pointer bg-gray-200 fill-black p-[10px] rounded-full dark:bg-gray-800 dark:fill-white">
      ...
      <div class="hidden justify-between items-center w-full md:flex md:w-auto md:order-1" id="mobile-menu-2">
        <ul class="flex flex-col p-4 mt-4 bg-gray-50 rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700 ease-out duration-300">
          <li>
            <button @click="() => this.$router.push('/')" class="block py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white" aria-current="page">Home</button>
          </li>
          <li>
