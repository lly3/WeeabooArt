  ...
  <div class="flex flex-col xl:flex-row relative" v-if=is_loading>
    ...
    <div class="left-side xl:w-9/12 w-full z-0">
      ...
      <div class="p-4 flex font-bold dark:text-white lg:ml-28 lg:mr-0 md:mx-12">
        <div v-if=!favorite @click=isFavorite class="flex items-center sm:mr-8 mr-5 space-x-1.5 hover:text-greenlogo cursor-pointer">
        ...
        <div v-else @click=isFavorite class="flex items-center sm:mr-8 mr-5 space-x-1.5 hover:text-greenlogo cursor-pointer">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-star-fill" viewBox="0 0 16 16">
              <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
            </svg>
            <p class="sm:block hidden">Favorited</p>
        </div>
  ...
  created() {
    this.$watch(
      ...
      async (toParams, previousParams) => {
        ...
        await this.$axios.get(`/post/favorited/${this.post.id}`, {
          headers: {
